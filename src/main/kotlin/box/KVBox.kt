...
import work.delsart.guixu.bean.BoxInfo
...
import work.delsart.guixu.utils.toByteArray
import work.delsart.guixu.utils.toDoubleArray
import work.delsart.guixu.utils.toFloatArray
import work.delsart.guixu.utils.toInt
import work.delsart.guixu.utils.toIntArray
import work.delsart.guixu.utils.toLong
import work.delsart.guixu.utils.toLongArray
...
private const val valueTypeIntArray: Byte = 9
private const val valueTypeLongArray: Byte = 10
private const val valueTypeFloatArray: Byte = 11
private const val valueTypeDoubleArray: Byte = 12

@OptIn(ExperimentalAtomicApi::class)
class KVBox @DoNotUseDirectly constructor(host: GuiXu, path: Path, name: String) :
    ...
    private var mapFile =
        host.autoIncreaseFileBuilder(Path(path, "$name-K2IdMap"), 4, FileAccessMode.RW)
    ...
    private fun initialMap() {
        ...
        while (cursor < length) {
            ...
            if (keyLength > buffer.size) buffer = ByteArray(keyLength)
    ...
    private fun convertDataToByteArray(data: Any, type: Byte): ByteArray {
        return when (type) {
            ...
            valueTypeIntArray -> (data as IntArray).toByteArray()
            valueTypeLongArray -> (data as LongArray).toByteArray()
            valueTypeFloatArray -> (data as FloatArray).toByteArray()
            valueTypeDoubleArray -> (data as DoubleArray).toByteArray()
    ...
    private fun convertByteArrayToData(data: ByteArray, type: Byte): Any {
        return when (type) {
            ...
            valueTypeString -> data.decodeToString()
            valueTypeByteArray -> data
            valueTypeIntArray -> data.toIntArray()
            valueTypeLongArray -> data.toLongArray()
            valueTypeFloatArray -> data.toFloatArray()
            valueTypeDoubleArray -> data.toDoubleArray()
            else -> error("$type can not be convert")
        }
    }
    ...
    fun putIntArray(key: String, data: IntArray) {
        putData(key, valueTypeIntArray, data)
    }

    fun putLongArray(key: String, data: LongArray) {
        putData(key, valueTypeLongArray, data)
    }

    fun putFloatArray(key: String, data: FloatArray) {
        putData(key, valueTypeFloatArray, data)
    }

    fun putDoubleArray(key: String, data: DoubleArray) {
        putData(key, valueTypeDoubleArray, data)
    }
    ...
    fun getIntArray(key: String): IntArray {
        return getData(key, valueTypeIntArray) as IntArray
    }

    fun getLongArray(key: String): LongArray {
        return getData(key, valueTypeLongArray) as LongArray
    }

    fun getFloatArray(key: String): FloatArray {
        return getData(key, valueTypeFloatArray) as FloatArray
    }


    fun getDoubleArray(key: String): DoubleArray {
        return getData(key, valueTypeDoubleArray) as DoubleArray
    }
    ...
    override fun getInfo(): BoxInfo {
        val info = super.getInfo()
        return BoxInfo(
            dataSize = info.dataSize,
            indexSize = info.indexSize + mapFile.fileSize
        )
    }
